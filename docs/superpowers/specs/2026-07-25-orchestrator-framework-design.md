...
ai/orchestrator-framework/
  ...
  generate.py                # reads agents/*.yaml + models.yaml(+local) -> writes agent files
  setup.sh                    # runs generate.py directly into each harness's global config location
  tests/
    test_queue.py
    test_generate.py
  README.md
...
Unlike a static dotfile (vimrc, tmux.conf), the generated agent files bake in
a resolved model id that depends on `models.local.yaml`, which can differ per
machine — so they are **not** committed to git or symlinked from a
repo-local build directory. Instead `generate.py` writes directly into place
at setup time, exactly like this repo's `claude/settings.json` generation
(templated via `jq` in `claude/setup.sh`, never symlinked from a static
copy):
...
python3 generate.py \
  ...
  --opencode-out ~/.config/opencode/agent \
  --cursor-out ~/.orchestrator-cursor-modes.json
```

This repo's root `setup.sh` gains a call to `ai/orchestrator-framework/setup.sh`,
mirroring the existing call to `claude/setup.sh`. There is no `dist/`
directory in the repo at all — output only ever exists at the target
locations on a given machine, regenerated by re-running `setup.sh`.
...
resolved `model`) into that harness's native shape, and writes the result
directly to the `--<harness>-out` path passed on the command line (see §1):

- **Claude Code** → one `<name>.md` file per agent in the `--claude-code-out`
  directory (default `~/.claude/agents`), YAML frontmatter (`name`,
  `description`, `tools`, `model`) + prompt body. Well-documented format,
  high confidence.
- **OpenCode** → equivalent markdown-with-frontmatter agent file per agent
  in the `--opencode-out` directory (default `~/.config/opencode/agent`).
  Close enough to Claude Code's convention that the adapter is structurally
  near-identical.
- **Cursor** → a single `modes.json` file (all 4 modes) written to
  `--cursor-out`, following Cursor's custom-mode schema (name, prompt, tool
  restrictions, model).
...
Cursor reads custom modes from a plain file the way the other two harnesses
read agent definitions. `generate.py` produces `modes.json` in Cursor's
documented schema as a best-effort artifact at whatever path
`--cursor-out` names; wiring it into a live Cursor install may require a
manual import step the first time, until verified against a real Cursor
instance. This is consistent with the earlier decision that Cursor is the
file-based-bridge, human-mediated harness rather than a fully automated
one.
...
- Unit tests for `generate.py` adapters: given a fixed `agents/*.yaml` +
  `models.yaml` fixture, run generation into a temp directory (via
  `--claude-code-out`/`--opencode-out`/`--cursor-out` pointed at `tmp_path`)
  and assert exact Claude Code/OpenCode output files; Cursor output checked
  structurally only (per the §5 caveat).
